...
def test_execute_rebalance_blocks_filled_order_with_only_broker_order_id(tmp_path):
    class FakeIB:
        def openTrades(self):
            return []
...
def test_execute_rebalance_identity_failure_precedes_completed_order(tmp_path):
    class FakeIB:
        def openTrades(self):
            return []
        ...
        def accountValues(self):
            return [SimpleNamespace(tag="CashBalance", currency="USD", value="2000")]

    def submit_order_intent(_ib, intent):
        if intent.symbol == "VOO":
            return SimpleNamespace(
                ...
                average_fill_price=100.0,
                raw_payload={
                    "order_key": "ibkr|account=U123|client_id=7|order_id=123"
                },
            )
        return SimpleNamespace(
            broker_order_id="124",
    ...
    _trade_logs, summary = execute_rebalance(
        ...
        {"VOO": 0.4, "QQQ": 0.4},
        {},
        {"equity": 2000.0, "buying_power": 2000.0},
        fetch_quote_snapshots=lambda _ib, symbols: {
            symbol: SimpleNamespace(last_price=100.0) for symbol in symbols
        },
        ...
        order_intent_cls=OrderIntent,
        translator=build_translator("en"),
        strategy_symbols=["VOO", "QQQ"],
        strategy_profile="tech_communication_pullback_enhancement",
        signal_metadata=_signal_metadata(
            {"VOO": 0.4, "QQQ": 0.4},
            risk_symbols=("VOO", "QQQ"),
            trade_date="2026-04-01",
            snapshot_as_of="2026-03-31",
        ),
        dry_run_only=False,
    ...
    assert summary["execution_status"] == "blocked"
    assert summary["no_op_reason"] == "order_identity_unavailable:QQQ"
    assert [order["symbol"] for order in summary["orders_filled"]] == ["VOO"]
    ...
    def fake_submit_order_intent(_ib, intent):
        ...
        if intent.side == "sell":
            return SimpleNamespace(
                ...
                raw_payload={"order_key": "ibkr|account=U123|client_id=7|order_id=1"},
            )
...
def test_execute_rebalance_blocks_same_day_repeat_via_execution_lock(tmp_path, monkeypatch):
    ...
    kwargs = dict(
        ...
        submit_order_intent=lambda *_args, **_kwargs: SimpleNamespace(
            broker_order_id="1",
            status="Submitted",
            raw_payload={"order_key": "ibkr|account=DU123|client_id=7|order_id=1"},
        ),
        order_intent_cls=OrderIntent,
        translator=translate,
        strategy_symbols=["VOO", "BOXX"],
...
def test_execute_rebalance_sells_cash_sweep_symbol_when_buying_power_is_short(monkeypatch, tmp_path):
    ...
    def fake_submit_order_intent(_ib, intent):
        ...
        return SimpleNamespace(
            ...
            status="Submitted",
            raw_payload={
                "order_key": f"ibkr|account=DU123|client_id=7|order_id={len(submitted)}"
            },
        )
