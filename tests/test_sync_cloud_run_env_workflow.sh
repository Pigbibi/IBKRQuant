...
grep -Fq '            --delete-legacy-schedulers' "$workflow_file"
grep -Fq 'HK verify scheduler migration requires the configured migration first.' "$workflow_file"
test "$(grep -Fc 'Skipping precheck scheduler migration for the isolated HK verify target.' "$workflow_file")" -eq 0
